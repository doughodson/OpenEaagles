...
//              previous pointers are maintained using a LinkedList::Item structure.
//
// Public members:
//
...
{
   DECLARE_SUBCLASS(LinkedList, Object)
...
public:
   LinkedList();
   LinkedList(const double values[], const unsigned int nv);
   LinkedList(const int values[], const unsigned int nv);
