...
IMPLEMENT_EMPTY_SLOTTABLE_SUBCLASS(LinkedList, "LinkedList")

LinkedList::LinkedList() : headP(nullptr), tailP(nullptr), num(0)
{
    STANDARD_CONSTRUCTOR()
}

LinkedList::LinkedList(const double values[], const unsigned int nv) : headP(nullptr), tailP(nullptr), num(0)
{
    STANDARD_CONSTRUCTOR()
...
LinkedList::LinkedList(const int values[], const unsigned int nv) : headP(nullptr), tailP(nullptr), num(0)
{
    STANDARD_CONSTRUCTOR()
...
void LinkedList::copyData(const LinkedList& org, const bool cc)
{
...
//------------------------------------------------------------------------------
unsigned int LinkedList::getIndex(const Object* const obj) const
{
...
//------------------------------------------------------------------------------
void LinkedList::addHead(Object* const obj)
{
...
//------------------------------------------------------------------------------
void LinkedList::addTail(Object* const obj)
{
    if (obj == nullptr) return;
    Item* d = new Item;
    d->value = obj;
...
//------------------------------------------------------------------------------
bool LinkedList::remove(const Object* const obj)
{
    bool ok = false;
    if (obj != nullptr) {
        Item* d = headP;
...
//      getNumberList().
//------------------------------------------------------------------------------
unsigned int LinkedList::getNumberList(double values[], const unsigned int max) const
{
    unsigned int n = 0;
...
unsigned int LinkedList::getNumberList(float values[], const unsigned int max) const
{
    unsigned int n = 0;
    for (const Item* p = getFirstItem(); p != nullptr && n < max; p = p->getNext() ) {
        const Object* p1 = p->getValue();
...
unsigned int LinkedList::getNumberList(int values[], const unsigned int max) const
{
    unsigned int n = 0;
    for (const Item* p = getFirstItem(); p != nullptr && n < max; p = p->getNext() ) {
        const Object* p1 = p->getValue();
...
//------------------------------------------------------------------------------
bool LinkedList::insert(LinkedList::Item* newItem, LinkedList::Item* refItem)
{
    bool ok = true;
    if (refItem != nullptr) {
        if (refItem == headP) {
...
//                 (Ownership passed to caller -- does not unref())
//------------------------------------------------------------------------------
Object* LinkedList::remove(LinkedList::Item* item)
{
...
//------------------------------------------------------------------------------
void LinkedList::addHead(LinkedList::Item* item)
{
...
//------------------------------------------------------------------------------
void LinkedList::addTail(LinkedList::Item* item)
{
    item->next = nullptr;
    item->previous = tailP;
...
//------------------------------------------------------------------------------
const Object* LinkedList::getPosition1(const unsigned int n) const
{
...
//------------------------------------------------------------------------------
std::ostream& LinkedList::serialize(std::ostream& sout, const int, const bool) const
{
    std::cout << "{" << std::endl;
