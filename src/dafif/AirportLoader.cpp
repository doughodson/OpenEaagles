...
// ---
   ...
   airport.setRecord(r);
   while (r != 0 && airport.formatCode() == 1) {

      // Is the airport in the correct country?
      int inArea = true;
      if ( country != 0 ) inArea = airport.isCountryCode(country);
...
// ---
   ...
   while (r != 0 && runway.formatCode() == 2) {
      ...
      if (n > 0) {
         ...
         AirportKey* apk = (AirportKey*) ql[0];
...
int AirportLoader::queryByFreq(const float freq)
{
   ...
   for (int i = 0; i < nrl; i++) {
      ...
      if (k->rng2 < mr2) {
         if ( chkIlsFreq( (AirportKey*) k, freq ) ) {
            ql[nql++] = k;
         }
      }
   }
...
int AirportLoader::queryByChannel(const int chan)
{
   ...
   for (int i = 0; i < nrl; i++) {
      ...
      k->rng2 = range2(k->lat,k->lon);
      if (k->rng2 < mr2) {
         if ( chkIlsChan( (AirportKey*) k, chan ) ) {
            ql[nql++] = k;
         }
      }
   }
...
int AirportLoader::queryIlsByIdent(const char* id)
{
   nql = 0;
   for (int i = 0; i < nrl; i++) {
      AirportKey* apk = (AirportKey*) rl[i];
      for (RunwayKey* rwk = apk->runways; rwk != 0; rwk = rwk->next) {
         for (IlsKey* ilsk = rwk->ils; ilsk != 0; ilsk = ilsk->next) {
            if (strncmp(ilsk->id,id,ILS_IDENT_LEN) == 0) ql[nql++] = ilsk;
         }
      }
   }
   return nql;
...
int AirportLoader::queryIlsBySubkey(const char* subkey)
{
   ...
   nql = 0;
   if (rwk != 0) {
      int len = (int)strlen(subkey);
      for (IlsKey* ilsk = rwk->ils; ilsk != 0; ilsk = ilsk->next) {
         if (strncmp(ilsk->key,subkey,len) == 0) ql[nql++] = ilsk;
      }
   }
...
{
   if (freq <= 0.0f || apk == 0) return true;
...
void AirportLoader::makeSimpleLinkedList()
{
   firstAirport = 0;
   if (nql > 0) {
      firstAirport = (AirportKey*) ql[0];
      AirportKey* lastAP = firstAirport;
      lastAP->next = 0;
      for (int i = 1; i < nql; i++) {
         lastAP->next = (AirportKey*) ql[i];
         lastAP = lastAP->next;
         lastAP->next = 0;
      }
   }
...
void AirportLoader::printResults(std::ostream& sout)
{
   ...
   for (int i = 0; i < nql; i++) {
      ...
      // print the AIRPORT record
      AirportKey* apk = (AirportKey*) ql[i];
      airport.setRecord( db->getRecord( apk->idx ) ) ;
      airport.printRecord(sout);
...
//------------------------------------------------------------------------------
AirportLoader::IlsKey::IlsKey(const long idx,
                                       const Ils& ils) : Key(idx)
{
   size = ILS_RECORD_LEN;
   lat = ils.latitude();
   lon = ils.longitude();
...
//------------------------------------------------------------------------------
AirportLoader::RunwayKey::RunwayKey(const long idx,
                                             const Runway& runway) : Key(idx)
{
   runway.key(key);
   rwlen = runway.length();
                           ...
                           const float magvar, const int loc)
{
   ...
   double dist = double(loc);

   // compute delta latitude (degrees)
   double dlat = dist * cos(thdg*DEG2RAD) * FT2DEG;

   // compute delta longitude (degrees)
   double dlon = dist * sin(thdg*DEG2RAD) * FT2DEG * cos(lat*DEG2RAD);
...
void printLatitude( char* buff, double lat )
{
   ...
   double dmin = (dlat - double(deg)) * 60.0;
   int min = int(dmin);

   double dsec = (dmin - double(min)) * 60.0;
   int sec = int(dsec);
...
void printLongitude( char* buff, double lon )
{
   ...
   double dmin = (dlon - double(deg)) * 60.0;
   int min = int(dmin);
