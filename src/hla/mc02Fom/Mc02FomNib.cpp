...
IMPLEMENT_PARTIAL_SUBCLASS(Nib,"Mc02FomNib")
EMPTY_SLOTTABLE(Nib)
...
bool Nib::setOutputPlayerType(const Simulation::Player* const p)
{
   ...
   if (ok) {
      ...
      if (p->isClassType(typeid(Simulation::AirVehicle))) {
         setClassIndex( Mc02FomIO::AIRCRAFT_CLASS );
      }
      else if (p->isClassType(typeid(Simulation::Missile))) {
         setClassIndex( Mc02FomIO::MISSILE_CLASS );
      }
      else {
         // Invalid type
         ok = false;
...
bool Nib::isPlayerStateUpdateRequired(const LCreal curExecTime)
{
    ...
    bool required = 
      isAttributeUpdateRequired(Mc02FomIO::WORLD_LOCATION_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::ENTITY_TYPE_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::ENTITY_IDENTIFIER_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::IS_FROZEN_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::DEAD_RECKONING_ALGORITHM_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::ORIENTATION_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::VELOCITY_VECTOR_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::ANGULAR_VELOCITY_VECTOR_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::ACCELERATION_VECTOR_BE_AI) ||
      isAttributeUpdateRequired(Mc02FomIO::FORCE_IDENTIFIER_PE_AI);
